/**
 * Adds or updates a remote groovy script to a Nexus server via the REST API.
 */
...
CliBuilder cli = new CliBuilder(
    usage: 'groovy addUpdateScript.groovy -u admin -p admin123 -f scriptFile.groovy [-n explicitName] [-h nexusUrl]')

cli.with {
  f longOpt: 'file', args: 1, required: true, 'Script file to send to Nexus'
  h longOpt: 'host', args: 1, 'Nexus host url (including port if necessary). Defaults to http://localhost:8081'
  ...
  u longOpt: 'username', args: 1, required: true, 'A User with permission to use the Nexus Script resource'
}
